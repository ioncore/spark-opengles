...
namespace DX9
{
	...
	void DX9QuadRenderer::render(const Group& group) {
	  ...
	  //std::cout << "Enter rendering" << std::endl;

		HRESULT hr = 0;
		int nb_part = group.getNbParticles();

		//std::cout << "DX9PrepareBuffers(group)" << std::endl;
		if (!DX9PrepareBuffers(group))
			return;

		//std::cout << "prepareBuffers(group)" << std::endl;
...
*/
        ...
        DX9Info::getDevice()->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_TEXTURE);
				...
				DX9Info::getDevice()->SetTextureStageState(0, D3DTSS_ALPHAARG1,  D3DTA_TEXTURE);
				DX9Info::getDevice()->SetTextureStageState(0, D3DTSS_ALPHAARG2,  D3DTA_DIFFUSE);
		...
		//std::cout << "bind buffers and draw" << std::endl;
		...
		// bind buffers and draw
		{
				...
				case TEXTURE_2D:
					{
					  ...
					  //std::cout << "texture buffer " << DX9TextureBuffer << " copying " << 4 * group.getNbParticles() << " texels from " << textureBuffer << std::endl;
				...
				case TEXTURE_3D:
					{
						...
						std::cout << "CRITICAL ERROR: unsupported 3D textures" << std::endl;
						exit(42);
					}
					break;
				...
				case TEXTURE_NONE:
					{
						...
						std::cout << "WARNING: non-textured particles" << std::endl;
					}
					break;
			}


			//std::cout << "vertex buffer " << DX9VertexBuffer << " copying " << 4 * group.getNbParticles() << " vertices from " << vertexBuffer << std::endl;
			...
			//std::cout << "color buffer " << DX9ColorBuffer << " copying " << 4 * group.getNbParticles() << " colors from " << colorBuffer << std::endl;
			...
			//std::cout << "index buffer " << DX9IndexBuffer << " copying " << 6 * group.getNbParticles() << " indices from " << indexBuffer << std::endl;
...
/*
      ...
      DX9VertexBuffer->GetDesc(&vdesc);
      std::cout << "Vertex buffer info: "
          << vdesc.Format << " "
          << vdesc.Pool << " "
          << vdesc.Size << " "
          << vdesc.Type << " "
      ...
      for(unsigned int i=0; i < 12; i++) {
        std::cout << (float)vertexBuffer[i].x << ";" << (float)vertexBuffer[i].y << ";" << (float)vertexBuffer[i].z << " ";
      }
      std::cout << std::endl;

      D3DVERTEXBUFFER_DESC cdesc;
      memset(&cdesc, 0, sizeof(cdesc));
      DX9ColorBuffer->GetDesc(&cdesc);
      std::cout << "Color buffer info: "
          << cdesc.Format << " "
          << cdesc.Pool << " "
          << cdesc.Size << " "
          << cdesc.Type << " "
      ...
      for(unsigned int i=0; i < 4*group.getNbParticles(); i++) {
        std::cout << (unsigned int)(colorBuffer[i]) << " ";
      }
      std::cout << std::endl;

      D3DVERTEXBUFFER_DESC tdesc;
      memset(&tdesc, 0, sizeof(tdesc));
      DX9ColorBuffer->GetDesc(&tdesc);
      std::cout << "Texel buffer info: "
          << tdesc.Format << " "
          << tdesc.Pool << " "
          << tdesc.Size << " "
          << tdesc.Type << " "
      ...
      for(unsigned int i=0; i < 8; i++) {
        std::cout << (float)(textureBuffer[i]) << " ";
      }
      std::cout << std::endl;

			D3DINDEXBUFFER_DESC idesc;
			memset(&idesc, 0, sizeof(idesc));
			DX9IndexBuffer->GetDesc(&idesc);
			std::cout << "Index buffer info: "
			    << idesc.Format << " "
			    << idesc.Pool << " "
			    << idesc.Size << " "
			    << idesc.Type << " "
			    << idesc.Usage << std::endl;
			//2400 bytes = 1200 shorts <=> 200 quads

			for(unsigned int i=0; i < 6*group.getNbParticles(); i++) {
			  std::cout << (unsigned int)(indexBuffer[i]) << " ";
			}
			std::cout << std::endl;
*/

//			std::cout << "draw primitives " << (int)nb_part << " " << (int)(nb_part<<2) << " " << (int)(nb_part<<1)
//			    << " \"" << group.getName() << "\" " << " " << &group << "; renderer " << this
//			    << std::endl;
			this->offsetIndex = 0;
			try{
        device->DrawIndexedPrimitive(
            ...
            nb_part<<2, //NumVertices. Number of vertices used during this call. The first vertex is located at index: BaseVertexIndex + MinIndex.
            ...
            nb_part<<1  //PrimitiveCount. Number of primitives to render. The number of vertices used is a function of the primitive count and the primitive type. The maximum number of primitives allowed is determined by checking the MaxPrimitiveCount member of the D3DCAPS9 structure.
            );
			} catch(...) {
		...
		//std::cout << "DX9QuadRenderer::render done!" << std::endl;
	}
	...
	bool DX9QuadRenderer::DX9CheckBuffers(const Group& group)
	{
		if( !DX9Bind(group, DX9_VERTEX_BUFFER_KEY, (void**)&DX9VertexBuffer) )
		{
			...
			DX9IndexBuffer = NULL;
			//std::cout << "Bind DX9_VERTEX_BUFFER_KEY failed" << std::endl;
			return false;
		}
		if( !DX9Bind(group, DX9_COLOR_BUFFER_KEY, (void**)&DX9ColorBuffer) )
		{
			DX9VertexBuffer = DX9ColorBuffer = DX9TextureBuffer = NULL;
			DX9IndexBuffer = NULL;
			//std::cout << "Bind DX9_COLOR_BUFFER_KEY failed" << std::endl;
			return false;
		}
		if( !DX9Bind(group, DX9_INDEX_BUFFER_KEY, (void**)&DX9IndexBuffer) )
		{
			DX9VertexBuffer = DX9ColorBuffer = DX9TextureBuffer = NULL;
			DX9IndexBuffer = NULL;
			//std::cout << "Bind DX9_INDEX_BUFFER_KEY failed" << std::endl;
			return false;
		}
		if( texturingMode != TEXTURE_NONE )
		{
			if( !DX9Bind(group, DX9_TEXTURE_BUFFER_KEY, (void**)&DX9TextureBuffer) )
			{
				DX9VertexBuffer = DX9ColorBuffer = DX9TextureBuffer = NULL;
				DX9IndexBuffer = NULL;
				//std::cout << "Bind DX9_TEXTURE_BUFFER_KEY failed" << std::endl;
				return false;
			}
		}
	...
	bool DX9QuadRenderer::DX9CreateBuffers(const Group& group) {
	  //std::cout << "DEBUG: DX9QuadRenderer::DX9CreateBuffers()" << std::endl;

		if( DX9Info::getDevice() == NULL ) {
		  //std::cout << "DEBUG: DX9QuadRenderer::DX9CreateBuffers did not find device" << std::endl;
		  return false;
		}

		//if( DX9Buffers.size() == 0 )
		if(pVertexDecl == 0 || pVertexDecl2D == 0 || pVertexDecl3D == 0)
		{
			SAFE_RELEASE( pVertexDecl );
			SAFE_RELEASE( pVertexDecl2D );
			SAFE_RELEASE( pVertexDecl3D );
...
////////////////////////////////////
		...
		// vertex buffer
		if( DX9Info::getDevice()->CreateVertexBuffer(group.getParticles().getNbReserved() * 4 * sizeof(D3DXVECTOR3), 0, D3DFVF_XYZ, D3DPOOL_DEFAULT, &vb, NULL) != S_OK ) {
		  //std::cout << "CRITICAL ERROR: CreateVertexBuffer vertex failed" << std::endl;
		  //exit(42);
		  return false;
		}
		//std::cout << this << " buffer created: "<< vb << std::endl;
		...
		// color buffer
		if( DX9Info::getDevice()->CreateVertexBuffer(group.getParticles().getNbReserved() * 4 * sizeof(D3DCOLOR), 0, D3DFVF_DIFFUSE, D3DPOOL_DEFAULT, &vb, NULL) != S_OK ) {
		  //std::cout << "CreateVertexBuffer color failed" << std::endl;
		  return false;
		}
		...
		if( DX9Info::getDevice()->CreateIndexBuffer(group.getParticles().getNbReserved() * 6 * sizeof(short), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &ib, NULL) != S_OK ) {
		  //std::cout << "CreateIndexBuffer failed" << std::endl;
		  return false;
		}
		...
		{
		case TEXTURE_2D :
			if( DX9Info::getDevice()->CreateVertexBuffer(group.getParticles().getNbReserved() * 8 * sizeof(float), 0, D3DFVF_TEX1, D3DPOOL_DEFAULT, &vb, NULL) != S_OK ) {
			  //std::cout << "CreateVertexBuffer texture failed" << std::endl;
			  return false;
			}
			...
			break;
		case TEXTURE_3D :
			if( DX9Info::getDevice()->CreateVertexBuffer(group.getParticles().getNbReserved() * 12 * sizeof(float), 0, D3DFVF_TEX1|D3DFVF_TEXCOORDSIZE1(3), D3DPOOL_DEFAULT, &vb, NULL) != S_OK ) {
			  return false;
			}
			key = std::pair<const Group *, int>(&group, DX9_TEXTURE_BUFFER_KEY);
			DX9Buffers[key] = vb;
  ...
  void DX9QuadRenderer::DX9CallTexture2DAtlas(const Particle& particle) const
  {
    ...
    *(textureIterator++) = textureAtlasV1();
    ...
    //std::cout << "DEBUG: destroying buffers " << &group << " " << DX9VertexBuffer << " " << DX9ColorBuffer << " " << DX9TextureBuffer << " " << DX9IndexBuffer << std::endl;
